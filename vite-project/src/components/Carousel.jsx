...
const Carousel = ({
  ...
  const settings = {
    ...
    autoplay,
    autoplaySpeed,
    nextArrow: nextArrow ? <NextArrow /> : null,
    prevArrow: prevArrow ? <PrevArrow /> : null,
      ...
      <Slider {...settings}>
        {categories.map((category, index) => (
          <div key={index} className="flex justify-center px-1">
            <div
              className={`text-white flex flex-col justify-center items-center p-4 ${bgClassName} rounded-lg h-32 border border-black-700`}
            >
              {category.image && (
                <img
                  src={category.image}
              ...
              <div className={`text-4xl mb-2 ${iconClassName}`}>
                {category.icon}
              </div>
