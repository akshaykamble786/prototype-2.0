...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="relative max-w-sm w-full sm:w-80 sm:max-h-[500px] flex sm:flex-col flex-row rounded-xl md:rounded-xs overflow-hidden shadow-lg bg-gray-200 p-2 mx-auto mb-3">
      {/* Image Section */}
      <div className="w-1/3 sm:w-full flex items-center justify-center overflow-hidden">
        <Link to={`/${categoryRoute}/${product.id}`} onClick={handleProductClick}>
          <img
            className="w-full h-auto sm:h-64 object-contain transition-transform duration-300 hover:scale-105 cursor-pointer"
            src={hasColors ? product.colors[selectedColor].images[0] : product.images[0]}
            alt={product.title}
          />
        </Link>
      </div>
        ...
        <div className="mt-2 mb-2 flex items-center space-x-2">
          {Array.from({ length: 5 }, (_, index) => (
            <svg
              ...
              stroke="currentColor"
              className={`w-4 h-4 ${
                index < Math.floor(product.rating) ? "text-yellow-300" : "text-gray-500"
              }`}
            >
              <path
                strokeLinecap="round"
